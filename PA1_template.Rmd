...
Note that these are (almost) the same values as above.  This does not appear to be an error, because if we calculate the mean manually (sum of steps divided by number of rows) for each you can see that despite the sums and rows being different the means are identical:
```{r Manually Calculate Means}
nrow(steps.total)
sum(steps.total[, 2])
